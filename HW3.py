...
trainer = Adadelta( lr = 0.05 , rho = 0.95)
...
for epoch in range(1):
	...
	for split in range(s):
		...
		label_vec = []
		for w in label[(train_num/s * split) : (train_num/s * (split+1))]:
			l[w] = 1
			label_vec.append([l])
			l = [0]*(word_num+1)
		label_vec = np.array(label_vec, dtype = theano.config.floatX)
		model.fit(train_vec , label_vec , batch_size = 128 , nb_epoch = 1 , validation_split=0.0 , shuffle=False)
...
for key in sorted(test):
	...
	ch = [result[-1][0][word_dict[c]] for c in test[key]['choice']]
